...
# Phase 1: add seeds for the strongest and most diagnostic settings.
# seed0 already exists in the current W&B project for these settings, so this
# batch adds three new seeds without duplicating existing runs.
primary_seeds=(1 2 3)
primary_specs=(
  "LN baseline"
  "RMS attn"
...
for seed in "${primary_seeds[@]}"; do
  for spec in "${primary_specs[@]}"; do
    ...
    generate_job "${norm}" "${slot}" "${seed}" "multiseed"
  done
done

# Phase 2: centering-only mechanism checks. These are new settings, so seed0 is
