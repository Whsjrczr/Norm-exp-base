...
dir_name=exp-task3-patch8-grid
...
lr_method=cos
seed=0
subjectname="ViT-SeqBN-patch8-grid"
...
norms=(LN RMS BN BNs SBN SBNs bCRMS SeqBN SeqBNs DSeqBN DSeqBNs DSeqBCRMS)
...
for norm in "${norms[@]}"; do
  for activation in "${activations[@]}"; do
    for lr in "${lrs[@]}"; do
      baseString="execute_${arch}_${dataset}_img${image_size}_patch${patch_size}_${norm}_${activation}_lr${lr}_bs${batchsize}_drop${dropout}_dpath${droppath}_wd${weightdecay}_s${seed}_${optimizer}"
