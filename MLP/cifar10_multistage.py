...
class MNIST:
    ...
    def _rebuild_optim_sched_and_sync_saver(self):
        """Rebuild optimizer/scheduler from current cfg, and keep checkpoint object in sync."""
        self.optimizer = ext.optimizer.setting(self.model, self.cfg)
        self.scheduler = ext.scheduler.setting(self.optimizer, self.cfg)
    ...
    def _epoch_stage_plan(self):
        """Return list of (stage_idx, stage_dict, epoch_begin, epoch_end_exclusive)."""
        ...
        for st in stages:
            ...
            # duration (epochs) or absolute end_epoch
            if 'end_epoch' in st:
        ...
        # If not covered until the end, extend with last stage config
        if cur < epochN:
            plan.append((si, stages[-1], cur, epochN))

        return plan

    def _apply_stage_to_cfg(self, st: dict | None):
        """Override cfg fields from stage dict."""
        if not st:
            return
        ...
        # Optional per-stage scheduler overrides
        if 'lr_method' in st:
    ...
    def train(self):
        ...
        for si, st, epoch_begin, epoch_end in stage_plan:
            ...
            # Rebuild optimizer/scheduler at the start of each segment
    ...
    def validate(self, epoch=-1):
        ...
        if self.cfg.arch == 'AE':
            # to_img is assumed defined elsewhere in your project, same as original cifar10.py
